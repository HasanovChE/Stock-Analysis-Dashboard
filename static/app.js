...
let forgotPasswordEmail = ''; // Store email across forgot/reset steps
...
async function handleForgotPassword() {
    ...
    try {
        ...
        forgotPasswordEmail = email; // Save for the reset step
...
async function handleResetPassword() {
    const email = forgotPasswordEmail; // Use the saved email from the forgot step
    const code = getEl('resetCode').value.trim();
    const new_password = getEl('newPassword').value;
    const errorEl = getEl('authError');

    if (!email) {
        errorEl.innerText = "Session expired. Please start the forgot password process again.";
        showAuthForm('forgot');
        return;
    }
    ...
    try {
        ...
        forgotPasswordEmail = ''; // Clear stored email
